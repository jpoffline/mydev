""" SQL query factories """
import app.lib.tools.datetimetools as dttools

def get_all_from_sql_qy(table, order=None, topn=None, what=None,where=None):
...
def select_table_name_from_db_qy(table):
    """ SQL-factory: select a particular table name from the database """
    return "SELECT name " + cat_from_tb('sqlite_master') + " " \
...
def select_groupby_time(table, meta, where=None):
    ...
    if others is not '':
        qy += ', ' + others
    qy += " " + cat_from_tb(table)
    qy += " GROUP BY " + strftime(time_res, timecol)
    if where is not None:
        qy += ' WHERE ' + where
...
def select_distinct_months(table, timecol):
    """
    Select the distinct months from a table
    """
...
def select_in_datetime(table, meta, datewanted, subagg=None):
    ...
    qy += " WHERE " + \
        strftime(meta['fmt'], meta['timecol']) + " = '" + datewanted + "'"
    if subagg is not None:
        qy += " GROUP BY " + \
            strftime(agglevel_to_format(subagg), meta['timecol'])
    return qy + ";"
...
def allowed_agg_levels():
    ...
    return dttools.allowed_strings()


def agglevel_to_format(agglevel):
    """ Aggregate level to format string """
    return dttools.to_fmtstring(agglevel)
